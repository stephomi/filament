...
utils::bitset64
static getLayoutBundleKey(const Program::SamplerGroupInfo& samplerGroupInfo) noexcept {
    utils::bitset64 key = {};
...
bool VulkanPipelineCache::bindDescriptors(VkCommandBuffer cmdbuffer) noexcept {
    ...
    if (bind) {
        auto layout = getOrCreatePipelineLayout();
...
void VulkanPipelineCache::getOrCreateDescriptors(
    ...
    // If this method has never been called before, we need to create a new pipeline layout.
    ...
    std::vector<VkDescriptorSet>* descriptorSetArenas = &layoutBundle->setArenas[0];
...
void VulkanPipelineCache::bindProgram(const VulkanProgram& program) noexcept {
    ...
    LayoutBundleKey key = getLayoutBundleKey(program.samplerGroupInfo);
    if (key != mLayoutKey) {
        markDirtyPipeline();
        markDirtyDescriptor();
        mLayoutKey = key;
...
bool VulkanPipelineCache::LayoutBundleEqual::operator()(const LayoutBundleKey& k1,
        const LayoutBundleKey& k2) const {
    return k1 == k2;
}
